...
class Pong_Ball {
 public:
   ...
   * @param type 0 represents collision with another atom, 1 collision with x
   * wall, 2 collision with y wall
   */
  void UpdateAfterCollision(Pong_Ball &second_ball, int type);

 private:
  /**
   * This class object is defined by its position and velocity vector declared
   * below
   */
  glm::vec2 position_;
  glm::vec2 velocity_;
