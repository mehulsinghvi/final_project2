...
Paddle::Paddle(glm::vec2 corner, glm::vec2 &position, glm::vec2 &velocity,
...
bool Paddle::CheckCollisionWithPaddle(finalproject::Pong_Ball &ball) const {
  ...
  bool collided = false;
  glm::vec2 future_pos = ball.GetPosition();
  if (abs(future_pos.x - position_.x - size_.x) <= radius_ &&
      (future_pos.y - position_.y - size_.y) <= radius_ &&
      (future_pos.y - position_.y) >= radius_) {
    ball.UpdateAfterCollision(ball, 1);
    collided = true;
  }
  if (abs(future_pos.x - position2_.x) <= radius_ &&
      (future_pos.y - position2_.y - size_.y) <= radius_ &&
      (future_pos.y - position2_.y) >= radius_) {
    ball.UpdateAfterCollision(ball, 1);
    collided = true;
  }
  if (collided) {
...
void Paddle::SlideLeftPaddle(int type) {
  if (type == 1 && position_.y >= corner_.y) {
    position_ -= velocity_;
  }
