...
CollisionLogic::CollisionLogic(glm::vec2 corner, glm::vec2 size, Paddle paddle)
  ...
  left_score_ = 0;
  right_score_ = 0;
  sound_wall_ = cinder::audio::Voice::create(
      cinder::audio::load(ci::app::loadAsset("pong_side.mp3")));
  sound_pad_ = cinder::audio::Voice::create(
      cinder::audio::load(ci::app::loadAsset("pong_pad.mp3")));
}
...
void CollisionLogic::BallCollisionsUpdate(Pong_Ball &ball,
                                          Pong_Ball &second) const {
...
bool CollisionLogic::IsStickyCollision(Pong_Ball &ball1, Pong_Ball &ball2,
                                       int type, glm::vec2 wall_vec) {
  float dotProduct;
  if (type == 1) {
...
void CollisionLogic::WallCollisionsUpdate(Pong_Ball &ball) {
  ...
  if (abs(future_pos.x - corner_.x) <= radius) {
    if (!CollisionLogic::IsStickyCollision(
            ball, ball, 2, glm::vec2(corner_.x, future_pos.y))) {
  ...
  if (abs(future_pos.y - corner_.y) <= radius) {
    if (!CollisionLogic::IsStickyCollision(
            ball, ball, 2, glm::vec2(future_pos.x, corner_.y))) {
...
size_t CollisionLogic::GetScore(player type) {
  if (type == left) {
    return left_score_;
  } else {
    return right_score_;
